...
interface WhiteSharedCardProps {
  title?: string;
  subtitle?: string;
  description?: string;
  badge?: string;
...
function WhiteSharedCard({ title, subtitle, description, badge, actions, className = "", children }: WhiteSharedCardProps) {
  const text = subtitle ?? description;
  const hasHeader = Boolean(title || badge || actions);

  return (
    <div className={'rounded-2xl border border-brand-border bg-brand-white p-6 ${className}'}>
      {hasHeader ? (
...
export default WhiteSharedCard;
